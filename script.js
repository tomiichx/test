...
$(document).on('click', '.clothing-menu-option-item-right', function (e) {
    ...
    if (canChange) {
        if (hasTracker && clothingCategory == "accessory") {
            $.post('https://flegma-butik/TrackerError');
            return
        } else {
            if (clothingCategory == "model") {
                $(inputElem).val(newValue);
                $.post('https://flegma-butik/setCurrentPed', JSON.stringify({ ped: newValue }), function (model) {
...
$(document).on('change', '.item-number', function () {
    ...
    if (hasTracker && clothingCategory == "accessory") {
        ...
        if (clothingCategory == "accessory" && inputVal == 13) {
            ...
            $.post('https://flegma-butik/updateSkinOnInput', JSON.stringify({
                clothingType: clothingCategory,
...
$(document).on('click', '.clothing-menu-header-camera-btn', function (e) {
    ...
    if (selectedCam == null) {
        ...
        $.post('https://flegma-butik/setupCam', JSON.stringify({
            value: camValue
        }));
        selectedCam = this;
    } else {
...
$(document).on('keydown', function () {
    switch (event.keyCode) {
        case 68: // D
            $.post('https://flegma-butik/rotateRight');
            break;
        case 65: // A
            $.post('https://flegma-butik/rotateLeft');
            break;
        case 38: // UP
            ChangeUp();
            break;
...
$(document).on('click', "#save-menu", function (e) {
    e.preventDefault();
    QBClothing.Close();
    $.post('https://flegma-butik/saveClothing');
});

$(document).on('click', "#cancel-menu", function (e) {
    e.preventDefault();
    QBClothing.Close();
    $.post('https://flegma-butik/resetOutfit');
});
...
QBClothing.Open = function (data) {
    ...
    $.each(data.menus, function (i, menu) {
        if (menu.selected) {
            $(".clothing-menu-header").append('<div class="clothing-menu-header-btn ' + menu.menu + 'Tab selected" data-category="' + menu.menu + '"><img class="itemIcon" src="icons/' + menu.label + '.svg"></div>')
            ...
            $(".clothing-menu-header").append('<div class="clothing-menu-header-btn ' + menu.menu + 'Tab" data-category="' + menu.menu + '"><img class="itemIcon" src="icons/' + menu.label + '.svg"></div>')
        }
...
QBClothing.Close = function () {
    $.post('https://flegma-butik/close');
    $(".container").css("display", "none");
    $(".camera-buttons").fadeOut(150);
...
QBClothing.SetMaxValues = function (maxValues) {
    $.each(maxValues, function (i, cat) {
        if (cat.type == "character") {
            ...
            $(itemMax).html("<p><span class='dick' data-tkey='item'>Item</span>: " + maxValues[containers.data('type')].item + "</p>");
            ...
            $(headerMax).html("<p><span class='dick' data-tkey='texture'>Texture</span>: " + maxValues[containers.data('type')].texture + "</p>");
...
$(document).on('click', '#save-outfit-save', function (e) {
    ...
    $.post('https://flegma-butik/saveOutfit', JSON.stringify({
...
$(document).on('click', '.change-camera-button', function (e) {
    ...
    $.post('https://flegma-butik/rotateCam', JSON.stringify({
        type: rotationType
    }))
});
